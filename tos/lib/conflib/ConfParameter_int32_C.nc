/**
 * ConfLib generic int32_t parameter module.
 *
 * @author Raido Pahtma
 * @license MIT
 */
#include "ConfLib.h"
generic configuration ConfParameter_int32_C(
		...
		int32_t value_min, int32_t value_default, int32_t value_max) {
	provides {
		interface Get<int32_t>;
		interface Set<int32_t>;
		interface GetSet<int32_t>;
		interface Changed<int32_t>;
	}
}
implementation {
	...
	components new IntegerConfParameterP(
		...
		CONF_PARAM_TYPE_SIGNED, int32_t) as Parameter;

	components new ConfParameterMeta_int32_C(value_min, value_default, value_max) as Meta;
